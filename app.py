...
MAX_ROWS_DESKTOP = 30_000
MAX_ROWS_MOBILE = 15_000

MAX_POINTS_DESKTOP = 10_000
MAX_POINTS_MOBILE = 2_000
...
def normalize_columns(cols):
    return [
        re.sub(r"[^\w]", "", c.replace("\ufeff", "").strip().lower())
        for c in cols
    ]
...
def home():
    ...
    if request.method == "POST":
        ...
        if df is not None:
            ...
            # ---------- MEMORY SAFETY ----------
            max_rows = MAX_ROWS_MOBILE if mobile else MAX_ROWS_DESKTOP
            if len(df) > max_rows:
            ...
            # ---------- SAMPLING FOR PLOTS ----------
            max_points = MAX_POINTS_MOBILE if mobile else MAX_POINTS_DESKTOP
            df_plot = df.sample(max_points, random_state=42) if len(df) > max_points else df
